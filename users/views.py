...
def profile(request, username):
    ...
    return render(request, 'users/profile-main.html', {'page_user': page_user,})
...
def profile_joined_challenges(request, username):
    page_user = get_object_or_404(User, username=username)
    books = page_user.profile.joined_challenges.all()
    ...
    if request.is_ajax():
        return render(request, 'users/joined-challenges.html', {'page_user':page_user, 'books':books})
